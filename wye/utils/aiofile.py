...
class AsyncFile(object):
    class _ReadContent:
        ...
        используйте _ReadContent().
        --- content для хранения возвращаемого значения
        --- more_body для отслеживания чанка при чтение файла
        """
        def __init__(
            self,
            content: Any = None,
    ...
    async def read(self) -> bytes:
        ...
        async with self._rw_lock:
            ...
            self._loop.run_in_executor(self._executor, self._read,
                                       _read_content, over_semaphore)
    ...
    async def write(
        ...
        async with self._rw_lock:
            over_semaphore = asyncio.Semaphore(0)
            self._loop.run_in_executor(self._executor, self._write,
                                       content, over_semaphore)
